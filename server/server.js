(function (global, factory) {
    ...
    // Load listings and comments from JSON files
    function loadDataFromFiles() {
        const dataDir = './data';
        const loadedData = {};
        const skipFiles = ['users.json']; // Skip files handled by protectedData
        
        try {
            if (fs__default['default'].existsSync(dataDir)) {
                const files = fs__default['default'].readdirSync(dataDir);
                
                for (const file of files) {
                    if (file.endsWith('.json') && !skipFiles.includes(file)) {
                ...
                console.warn(`Data directory ${dataDir} not found. Using default seed data only.`);
            }
        } catch (err) {
    ...
    const fileData = loadDataFromFiles();
    ...
    // Merge file data into seedData (file data takes precedence)
    Object.assign(seedData, fileData);
